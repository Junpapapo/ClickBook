...
| 13 | **多言語対応** | Chrome i18n API および内蔵多言語システムにより 7 言語（英語・韓国語・日本語・ドイツ語・スペイン語・繁体字中国語・簡体字中国語）に完全対応 |
...
| 21 | **インタラクティブ・マインドマップ (Premium)** | フォルダとブックマークの全体構造を、インタラクティブなノードベースのマインドマップ形式で可視化します。ノードの手動ドラッグ＆ドロップ座標を永久的に保存・復元する機能、ローカルAI（Gemini Nano）を活用したブランチの自動拡張・要約・翻訳機能、ツールバーや設定パネルに組み込まれた絵文字＆アイコンピッカーの連携に対応しています。 |
...
| 24 | **スマートカレンダー連携 (Premium)** | Todoリストおよびメモ機能と有機的に連携するスマートカレンダー。Nager.Date APIを活用した国別（日本/アメリカ/韓国など）の祝日のリアルタイム自動同期、月/週/日ビューの切り替え、HTML5ドラッグ＆ドロップによる予定や時間の変更、各種繰り返し予定（毎日/毎週/毎月など）の設定に対応。 |
| 25 | **アナログ・スプリングノート (Premium)** | アナログ書斎感性のリッチテキスト（Tiptapベース）ノートパッドです。ライト、グリッド、セピア, ダークの4つのビジュアルテーマに対応しており、テーマ変更時にエディタの外側背景、本の背表紙の立体陰影、区切り線が選択されたテーマに合わせて自動的かつ有機的に同期されます。 |
| 26 | **ClickBookバディ＆フォーカスタイマー (Premium)** | 可愛いアニメーションデスクペット「バディ」とともにポモドーロタイマーで集中を促します。バディをクリックすると9つのクイックアクションの円形ダイアルメニューが表示されます: **翻訳**（ドラッグしたテキストを即座に翻訳）、**ブックマーク**（現在のページを1クリックで保存）、**AI対話**（Gemini Nanoチャットパネル）、**タイマー**（ポモドーロ即時開始）、**休憩**（全画面ヒーリングモード）、**メモ**（付箋メモ）、**ToDo**（チェックリスト）、**バディ非表示**（現在のドメインで非表示）、**バディ設定**（サイズ・不透明度・テーマのカスタマイズ）。ランク別バディ図鑑（ベーシック、プレミアム、隠しバディ）および完了時の没入型ビジュアルテーマ（オーロラの星空、静かな針葉樹林、暖かな暖炉、夕暮れの湖畔、シンギングボウル瞑想ヨガ、全18話のシネマティック成功ストーリーカード）も提供します。 |
| 27 | **スマート初声検索** | 検索バー、ブックマーク一覧、モーダルフィルターにて韓国語の初声（例：「클릭북」→「ㅋㄹㅂ」）による超高速マッチング検索に対応 |
