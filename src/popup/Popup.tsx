...
export default function Popup() {
  ...
  return (
    <div className="w-96 bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100 p-3.5 flex flex-col gap-2.5 border border-slate-200/90 dark:border-slate-800 shadow-figma-lg transition-colors font-sans select-none">
      {/* Header */}
      <div className="flex items-center justify-between px-0.5">
        <div className="flex items-center gap-2">
          <img src="/icons/icon128.png" alt="ClickBook" className="w-5 h-5 rounded-md shadow-figma-sm" />
          <h1 className="text-sm font-bold tracking-tight relative flex items-center gap-1.5 text-slate-800 dark:text-slate-100">
            <span>ClickBook</span>
            <span className="flex items-center gap-0.5 bg-indigo-600 text-white px-1.5 py-0.5 rounded-full text-[9px] font-extrabold select-none leading-none shadow-figma-sm">
      ...
      {/* 북마크 섹션 */}
      <p className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-wider font-bold -mb-0.5">{t("popupBookmarkSection")}</p>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex gap-1.5">
          <button
            ...
            className="flex-1 figma-btn-primary py-2"
          >
            {status === "loading" ? <Loader2 size={12} className="animate-spin" /> : <BookmarkPlus size={12} />}
            {t("popupSave")}
          </button>
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-medium text-xs shadow-figma-sm transition-all duration-150 border border-indigo-400/20 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
          >
            {isReaderLoading ? <Loader2 size={12} className="animate-spin" /> : <BookOpen size={12} />}
            {t("offlineReaderBtn")}
          </button>
          <button
            ...
            className="flex-1 figma-btn-secondary py-2"
          >
        ...
        <div className="flex gap-1.5">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-xs font-medium transition-all cursor-pointer border ${
              textImportOpen
                ? "bg-indigo-600 text-white border-indigo-600 shadow-figma-sm"
                : "bg-slate-100 dark:bg-slate-800 hover:bg-slate-200/80 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 border-slate-200/80 dark:border-slate-700/80"
            }`}
          >
          ...
          <div className="flex-1 flex gap-1">
            ...
            <button
              ...
              className={`px-2.5 rounded-md transition-all cursor-pointer flex items-center justify-center border ${
