...
export default function SpringNoteShortcutGuide({ onClose, lang: propLang }: Props) {
  const { lang: ctxLang, t } = useLang();
  const lang = propLang || ctxLang || "ko";
  const isKo = lang === "ko";
  const isJa = lang === "ja";
  ...
  const shortcuts = [
    {
      keys: ["Ctrl", "B"],
      label: isKo ? "텍스트 굵게" : isJa ? "太字" : "Bold Text",
      desc: isKo ? "선택한 텍스트를 굵게 만듭니다." : isJa ? "選択したテキストを太字にします。" : "Make selected text bold."
    },
    {
      keys: ["Ctrl", "I"],
      label: isKo ? "텍스트 기울임" : isJa ? "斜体" : "Italic Text",
      desc: isKo ? "선택한 텍스트를 기울입니다." : isJa ? "選択したテキストを斜体にします。" : "Make selected text italic."
    },
    {
      keys: ["Ctrl", "U"],
      label: isKo ? "텍스트 밑줄" : isJa ? "下線" : "Underline Text",
      desc: isKo ? "선택한 텍스트에 밑줄을 긋습니다." : isJa ? "選択したテキストに下線を引きます。" : "Underline selected text."
    },
    {
      keys: ["Ctrl", "Shift", "X"],
      label: isKo ? "텍스트 취소선" : isJa ? "取り消し線" : "Strikethrough",
      desc: isKo ? "선택한 텍스트에 취소선을 긋습니다." : isJa ? "選択したテキストに取り消し線を引きます。" : "Strikethrough selected text."
    },
    {
      keys: ["Ctrl", "Alt", "C"],
      label: isKo ? "코드 블록 생성" : isJa ? "コードブロック作成" : "Code Block",
      desc: isKo ? "선택 영역을 소스코드 블록으로 감쌉니다." : isJa ? "選択範囲をコードブロックで囲みます。" : "Wrap selection in a code block."
    },
    {
      keys: ["Alt", "S"],
      label: isKo ? "노트 수동 저장" : isJa ? "ノート手動保存" : "Save Note",
      desc: isKo ? "현재 작성된 모든 노트 내용을 데이터베이스에 저장합니다." : isJa ? "作成されたすべてのノート内容を保存します。" : "Instantly save all content to the database."
    },
    {
      keys: ["Shift", "Enter"],
      label: isKo ? "강제 줄바꿈" : isJa ? "強制改行" : "Soft Line Break",
      desc: isKo ? "문단을 나누지 않고 현재 줄에서 개행(줄바꿈)만 수행합니다." : isJa ? "段落を分けずに現在の行で改行します。" : "Insert a line break without creating a new paragraph."
    },
  ];
  ...
  const features = [
    {
      ...
      desc: isKo 
        ...
        : isJa
        ? "本文に「/」を入力して表の挿入、チェックリスト、見出し、AIアシスタントを呼び出します。"
    ...
    },
    {
      ...
      title: isKo ? "이미지 클립보드 붙여넣기 & 드롭" : isJa ? "画像貼り付け＆ドロップ" : "Instant Image Paste & Drop",
      desc: isKo 
        ? "복사한 이미지를 Ctrl + V로 본문에 바로 붙여넣거나 외부 이미지 파일을 드래그 앤 드롭해 바로 삽입할 수 있습니다." 
        : isJa
        ? "Ctrl + Vで画像を直接貼り付けるか、外部画像ファイルをドラッグ＆ドロップして挿入できます。"
    ...
    },
    {
      ...
      title: isKo ? "캔버스 빈 곳 더블 클릭" : isJa ? "キャンバスの空き領域ダブルクリック" : "Double-Click on Canvas",
      desc: isKo 
        ? "노트 바깥쪽의 빈 모눈 영역을 더블 클릭하면 그 자리에 즉시 드로잉(메모/그림판) 노드를 생성하여 자유롭게 필기합니다." 
        : isJa
        ? "ノートの外側の空きグリッド領域をダブルクリックすると、その場に手書き描画ノードを生成して自由に筆記できます。"
        ...
        <div className="flex items-center justify-between gap-3 pb-4 border-b border-slate-200 dark:border-white/10 mb-5 shrink-0">
          <div className="flex items-center gap-2 text-amber-500">
            ...
            <h3 className="text-sm font-black tracking-wide uppercase text-slate-900 dark:text-white">
              {isKo ? "스프링노트 단축키 & 기능 가이드" : isJa ? "スプリングノート ショートカット＆ガイド" : "Spring Note Shortcuts & Guide"}
            </h3>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="space-y-6">
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-3 flex items-center gap-1.5">
              ...
              {isKo ? "텍스트 편집 단축키" : isJa ? "テキスト編集ショートカット" : "Editor Hotkeys"}
            </h4>
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-3 flex items-center gap-1.5">
              ...
              {isKo ? "스마트 필기 기능" : isJa ? "スマート筆記機能" : "Core Writing Features"}
            </h4>
        ...
        <div className="mt-6 pt-4 border-t border-slate-200 dark:border-white/5 flex items-center justify-center">
          <p className="text-[9px] text-slate-400 dark:text-slate-500 font-semibold tracking-wider uppercase text-center">
            {isKo 
              ...
              : isJa
              ? "💡 マウスなしでキーボードとドラッグだけで豊かな手書き体験をお楽しみください。"
