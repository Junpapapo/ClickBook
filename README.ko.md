...
| 13 | **다국어 지원** | Chrome i18n API 및 내장 다국어 시스템을 통해 7개 언어 (영어, 한국어, 일본어, 독일어, 스페인어, 중국어 번체, 중국어 간체) 완벽 대응 |
...
| 26 | **ClickBook 버디 & 포커스 타이머 (Premium)** | 귀여운 디지털 펫 버디와 함께 뽀모도로 타이머로 집중을 유도합니다. 버디를 클릭하면 9가지 빠른 기능의 원형 다이얼 메뉴가 팝업됩니다: **번역** (드래그 텍스트 즉시 번역), **북마크** (현재 페이지 1클릭 저장), **AI 대화** (Gemini Nano 채팅 패널), **타이머** (뽀모도로 즉시 시작), **휴식** (전체화면 힐링 모드), **메모** (스티키 메모), **할 일** (체크리스트), **숨기기** (현재 도메인에서 숨김), **설정** (크기·투명도·테마 커스터마이징). 버디 도감(베이직, 프리미엄, 히든) 및 몰입 완료 효과 테마(오로라 별밤, 고요한 전나무 숲, 타오르는 벽난로, 노을빛 호숫가, 싱잉볼 명상 요가, 18종 시네마틱 성공 스토리 카드)도 제공합니다. |
| 27 | **스마트 초성 검색** | 검색 창, 북마크 목록, 모달 필터에서 한글 초성(예: '클릭북' → 'ㅋㄹㅂ')만으로도 초고속 매칭 검색 지원 |
