...
| # | 기능 | 설명 |
|---|---|---|
| 1 | **1클릭 저장** | 확장 프로그램 아이콘 클릭 → 활성 탭 즉시 저장 |
| 2 | **AI 자동 분류** | Chrome Gemini Nano(`window.ai`)로 URL·제목을 분석하여 최적의 폴더로 분류 |
| 3 | **AI 자동 정리** | 사이드바의 "AI 자동 정리" 버튼으로 모든 북마크를 일괄 재분류 (실행 전 백업 자동 저장) |
| 4 | **계층형 폴더** | 무한 중첩 지원. 생성·이름 변경·이동·삭제·이모지 아이콘 설정 가능 |
| 5 | **드래그 앤 드롭** | 북마크 → 폴더, 폴더 → 폴더 간의 직관적인 이동 지원 |
...
| 9 | **완전 오프라인** | `chrome.storage.local`을 사용하여 완전 로컬 작동 (외부 서버 불필요) |
| 10 | **다국어 지원** | Chrome i18n API를 사용하여 한국어, 영어, 일본어 다국어 완벽 대응 |
